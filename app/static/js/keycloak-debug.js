...
class KeycloakDebugger {
    ...
    monitorFormSubmissions() {
        document.addEventListener('submit', (event) => {
            ...
            // Special handling for Keycloak sync forms
            if (form.action && form.action.includes('sync_users_from_keycloak')) {
                this.log('Keycloak Sync Form Submission Detected:', {
                    action: form.action,
                    method: form.method,
                    csrf_token: data.csrf_token ? 'present' : 'missing',
                    timestamp: new Date().toISOString()
                });
            }
        });
